...
1. **One signer.** Generalization to anyone else's hands was poor.
...
3. **One letter at a time.** No way to recognize actual phrases. The
...
yield the target phrase. The training objective is to maximize this
...
2. **KL-to-uniform label smoothing** (weight 0.1). I added a secondary
   ...
   distribution and the uniform distribution. This penalizes peaky
...
went from `""` to `"e"` to `"eee"` to recognizable garbage to actual
words. The collapse was broken.

## My second attempt was working but agonizingly slow
...
sequence from disk, slice the columns we want, normalize, return.
...
17 seconds/epoch**, about a 200x speedup. The bottleneck moved from
...
treating zero values as "missing" landmarks. MediaPipe normalizes
...
landmarks with absent ones, and after wrist-centering (which subtracts
...
1. Fixing the data pipeline (Parquet -> .npz): 200x speed-up, no CER
...
Input: (B, T, 390) landmark sequences, padded to T_max in batch
   |
   v
Conv1d stem (3x5 kernels, BN, GELU) - smooths jittery frames
   |
   v
Sinusoidal positional encoding
   |
   v
Transformer encoder x 12 layers (d_model=512, nhead=8, FFN=1024, GELU, pre-LN)
   |
   v
...
The PyTorch to ONNX export had its own series of trouble:
...
service. The normalization formula has to match exactly. The vocab has
