...
- **Different MediaPipe initialization flags?** Same offset.
...
projection math were the bug, both colors would be equally wrong. If
...
(640x480 in our case). That gets handed to MediaPipe.
...
In our app the `<video>` was rendered into a 527x394 box (CSS-scaled
from the 640x480 stream). So MediaPipe was getting a 527x394 input,
not 640x480. Different input means different model output means different
landmark coordinates.

The model still tracked the hand correctly within the downsampled
image, but the resulting normalized coordinates were in the
**527x394 space**, not the **640x480 space** that the user's actual
...
meant the displayed size depended on layout. Different layout means different
input dimensions means different landmark coordinates.
