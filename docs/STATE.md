...
| 127-landmark feature packing + normalisation | ✅ `lib/landmarks.ts`. Strongly typed (group-named, not positional). Validation harness in `lib/__tests__/`. |
...
| Phrase transcription | ⚠️ **Mid-migration.** Currently hold-to-record (`useTranscribe` → POST `/api/transcribe-landmarks`). About to be replaced with always-on streaming. |
...
1. **`useMediaPipe` shared producer hook** — currently `useSignDetection`
   ...
   `detectFrame()` independently, paying 2× the MediaPipe cost per frame.
...
```
model/
  landmarks.py                 # 127-landmark selection + normalisation (source of truth)
