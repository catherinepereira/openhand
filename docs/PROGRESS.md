...
87K labeled per-letter images of one signer, A-Z + DEL/NOTHING/SPACE.
...
3. Wrist-center + 95th-percentile-scale the landmark vector
4. Save as a single `X.npy` / `y.npy` pair: **62,819 samples x 63 floats**
...
- **One signer in training** -> poor generalization
...
2. **KL-to-uniform label smoothing** (weight 0.1): penalizes peaky
...
After this fix:
- **Epoch time dropped from 57 min to 17 seconds** (~200x speedup)
...
it was treating zero-valued landmarks as "missing." But MediaPipe normalizes
...
missing data, and after wrist-centering (which writes zeros), legitimate
...
2. **Time stretch** (0.85-1.15x speed via frame resampling): different
   signing speeds
...
1. **Fixing the broken data pipeline** (per-Parquet re-reads -> per-sequence .npz):
   200x speed, no CER change but enabled much longer experiments
