...
### 8 · CER function isn't normalised consistently
...
### 15 · Two parallel landmark/normalisation implementations

- **A**: `openhand-model/model/landmarks.py` and
  `openhand/backend/services/ctc_landmarks.py` are duplicate
  implementations of the same 127-landmark spec + normalisation. They
...
- **N**: New client per request → new TCP/TLS handshake per request.
...
- **N**: If the WebSocket fails, the 100ms interval keeps trying to
  ...
  be nicer. **Optional — current behaviour is acceptable.**
...
## Suggested action plan, prioritised
...
2. **Centralise the backend URL** in one place in the frontend (~5 min).
...
The rest are either optional polish or intentional behaviour worth
