"""
CTC fingerspelling classifier wrapping the ONNX-exported transformer.

Input:  (T, N_FEATURES) float32 numpy array (one signed phrase, T frames)
...
because it typically gets a 0.03-0.05 CER reduction over greedy.
"""
...
def _beam_search_decode(
    ...
    """
    Standard CTC prefix beam search. log_probs shape: (T, V), blank at last index.

    Each beam tracks two probabilities for a given prefix:
      p_b:  prefix ends in a blank
      p_nb: prefix ends in a non-blank
    ...
    for t in range(T):
        ...
        for prefix, (p_b, p_nb) in beams.items():
            for s in top_idx:
                ...
                if s == last:
                    # Repeat: from blank we extend, from non-blank we collapse.
                    new_prefix = prefix + (s,)
...
class CTCClassifier:
    ...
    def transcribe(
        ...
        """features: (T, N_FEATURES) raw (un-normalized, zero-filled missing) frames.
        ...
        # The dynamo-exported ONNX needs batch >= 2 to keep the batch axis
